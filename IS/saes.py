...
if __name__ == "__main__":
    # Prompt the user to enter the 16-bit key
    key_input = input("Enter 16-bit key in binary format: ")
    key = int(key_input, 2)  # Convert the binary string to an integer

    # Create an instance of SimplifiedAES with the provided key
    aes = SimplifiedAES(key=key)

    # Prompt the user to enter the 16-bit plaintext
    plaintext_input = input("Enter 16-bit plaintext in binary format: ")
    plaintext = int(plaintext_input, 2)  # Convert the binary string to an integer
